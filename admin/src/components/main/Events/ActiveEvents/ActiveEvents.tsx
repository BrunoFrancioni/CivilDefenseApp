...
const ActiveEvents = () => {
    ...
    const getEvents = async () => {
        try {
            const result = await eventsService.getPaginateActiveEvents(actualPage, sizePage);
    ...
    const handleFinishClick = (id: string) => {
        Swal.fire({
            title: '¿Está seguro que desea dar terminar el evento?',
            showCancelButton: true,
            confirmButtonText: 'Terminar'
        }).then(async result => {
            if (result.isConfirmed) {
                try {
                    const result = await eventsService.setEventInactive(id);
                    console.log(result);

                    Swal.fire({
                        title: 'Evento terminado correctamente',
                        icon: 'success'
                    }).then(() => {
                        setLoading(true);
                        setTotalResults(0);
                                        ...
                                        <tr key={event._id}>
                                            ...
                                            <td>
                                                <div className="container-options">
                                                    ...
                                                    <span title="Dar por terminado el evento">
                                                        <i
                                                            className="fas fa-arrow-down option"
                                                            onClick={() => handleFinishClick(event._id)}
                                                        ></i>
                                                    </span>
